...
Version: 1.0 
*/
define(["webupload","jquery"],function(WebUploader,$){
    $.fn.extend({
		/*
		*	上传方法 opt为参数配置;
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			webUploader.on('uploadError',function( file, reason ){
				var $fileBox = $('#fileBox_'+file.id);
				console.log(webUploader.getFiles('error'));
				for(var i=0;i<webUploader.getFiles('error').length;i++){
		webUploader.removeFile(webUploader.getFiles('error')[i].id,true);
				};
				var $diyBar = $fileBox.find('.diyBar');	
...
//			$parentFileBox.append( div );
		...
		var li = '<li id="fileBox_'+file_id+'" class="diyUploadHover"> \
					<button class="del-pic icon">&#xe616;</button> \
		...
		//父容器宽度;
		var $width = $('.fileBoxUl>li').length * 215;
